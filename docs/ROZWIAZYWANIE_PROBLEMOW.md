...
```bash
az resource show \
  --ids "/.../providers/Microsoft.Web/sites/dvlp-ksef-func" \
...
```bash
# 1. Lista funkcji
az functionapp function list --name dvlp-ksef-func --resource-group rg-ksef --query "[].name" -o tsv
...
az monitor app-insights query --app dvlp-ksef-func --analytics-query \
...
# App Settings
az functionapp config appsettings list --name dvlp-ksef-func --resource-group rg-ksef -o table

...
This document describes the 38 new Power Platform Custom Connector operations added for the Self-Billing module. All operations require OAuth 2.0 authentication via Entra ID and respect RBAC roles (Admin / Approver / User).

---

## Table of Contents

1. [Suppliers (10 operations)](#1-suppliers)
2. [Self-Billing Agreements (7 operations)](#2-self-billing-agreements)
3. [Self-Billing Templates (6 operations)](#3-self-billing-templates)
4. [Self-Billing Invoices & Workflow (11 operations)](#4-self-billing-invoices--workflow)
5. [Self-Billing Import (3 operations)](#5-self-billing-import)
6. [Timer Trigger (1 background)](#6-timer-trigger)
7. [Power Automate Examples](#7-power-automate-examples)
8. [Copilot Studio Agent Operations](#8-copilot-studio-agent)
...
|---|---|---|---|---|---|
| 1 | `ListSuppliers` | GET | `/suppliers` | User | List suppliers with filters. Requires `settingId`. |
| 2 | `CreateSupplier` | POST | `/suppliers` | Admin | Create a new supplier. NIP must have valid checksum. |
| 3 | `GetSupplier` | GET | `/suppliers/{id}` | User | Get supplier details by ID. |
...
| 6 | `GetSupplierStats` | GET | `/suppliers/{id}/stats` | User | Invoice stats (count, totals, overdue). |
| 7 | `RefreshSupplierStats` | POST | `/suppliers/{id}/stats/refresh` | Admin | Force recalculation of cached stats. |
| 8 | `GetSupplierInvoices` | GET | `/suppliers/{id}/invoices` | User | List invoices for a specific supplier. |
...
| # | operationId | Method | Route | Role | Description |
|---|---|---|---|---|---|
| 1 | `ListSbAgreements` | GET | `/sb-agreements` | User | List agreements. Requires `settingId`. |
| 2 | `CreateSbAgreement` | POST | `/sb-agreements` | Admin | Create agreement. Supplier must exist and be active. |
| 3 | `GetSbAgreement` | GET | `/sb-agreements/{id}` | User | Get agreement details. |
...
| 6 | `ListSbAgreementAttachments` | GET | `/sb-agreements/{id}/attachments` | User | List agreement attachments. |
...
| # | operationId | Method | Route | Role | Description |
|---|---|---|---|---|---|
| 1 | `ListSbTemplates` | GET | `/sb-templates` | User | List templates. Requires `settingId`. |
| 2 | `CreateSbTemplate` | POST | `/sb-templates` | Admin | Create a new invoice item template. |
| 3 | `GetSbTemplate` | GET | `/sb-templates/{id}` | User | Get template details. |
...
| # | operationId | Method | Route | Role | Description |
|---|---|---|---|---|---|
| 1 | `ListSelfBillingInvoices` | GET | `/invoices/self-billing` | User | List with filters. Requires `settingId`. |
| 2 | `CreateSelfBillingInvoice` | POST | `/invoices/self-billing` | Admin | Create a single self-billing invoice. |
| 3 | `PreviewSelfBillingInvoice` | POST | `/invoices/self-billing/preview` | User | Preview without saving. |
...
| 10 | `SellerRejectInvoice` | POST | `/invoices/self-billing/{id}/reject` | Approver | PendingSeller → SellerRejected (reason required). |
...
| 3 | `DownloadSelfBillingTemplate` | GET | `/invoices/self-billing/import/template` | User | Download CSV/XLSX template file. |

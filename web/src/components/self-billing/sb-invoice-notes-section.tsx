...
import { api, Note, NoteCreate, NoteUpdate, queryKeys } from '@/lib/api'
import { useSbInvoiceNotes } from '@/hooks/use-api'
...
interface SbInvoiceNotesSectionProps {
  sbInvoiceId: string
  defaultExpanded?: boolean
  isReadOnly?: boolean
}

export function SbInvoiceNotesSection({
  sbInvoiceId,
  defaultExpanded = false,
  isReadOnly = false,
}: SbInvoiceNotesSectionProps) {
  const queryClient = useQueryClient()
  const { toast } = useToast()
  const t = useTranslations('selfBilling.notes')
  ...
  const {
    ...
    error: notesError,
  } = useSbInvoiceNotes(sbInvoiceId, isExpanded)

  // Create note mutation
  const createNoteMutation = useMutation({
    mutationFn: (data: NoteCreate) => api.selfBilling.createNote(sbInvoiceId, data),
    onSuccess: () => {
  ...
  // Update note mutation (reuses shared /api/notes/:id)
  ...
  // Delete note mutation (reuses shared /api/notes/:id)

...
import { useForecastMonthly } from '@/hooks/use-api'
...
export function ForecastTile({ delay = 0 }: ForecastTileProps) {
  ...
  const { data, isLoading } = useForecastMonthly(
    { horizon: 6, settingId: selectedCompany?.id },
  )
          ...
          <CardHeader className="pb-2 pt-4 px-5">
            <CardTitle className="flex items-center justify-between text-sm font-medium">
              <span className="flex items-center gap-2">
                ...
                {intl.formatMessage({ id: 'dashboard.tiles.forecast6Months' })}
              </span>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {intl.formatMessage({ id: 'dashboard.tiles.confidence' })}: {(confidence * 100).toFixed(0)}%
                  </span>
                </div>
              </div>
            </div>

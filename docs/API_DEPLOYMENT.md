...
Dokument opisuje proces przygotowania, przeprowadzenia i weryfikacji wdrożenia API (`dvlp-ksef-func`) na platformę Azure Functions w planie Flex Consumption.
...
| Deployment Storage | `dvlpksefstorage` (blob: `app-package-dvlp-ksef-func-*`) |
...
```bash
cd api
func azure functionapp publish dvlp-ksef-func
```

`func` CLI automatycznie:
1. Tworzy archiwum ZIP z katalogu `api/` (z pominięciem `.funcignore`)
2. Uploaduje paczkę do blob storage (`dvlpksefstorage`)
...
Functions in dvlp-ksef-func:
    health - [GET] https://dvlp-ksef-func-....azurewebsites.net/api/health
    healthDetailed - [GET] https://dvlp-ksef-func-....azurewebsites.net/api/health/detailed
    ... (62 funkcje)
```
...
**Objaw:** Deploy raportował sukces, `func` CLI wyświetlał "The deployment was successful!", ale sekcja "Functions in dvlp-ksef-func:" była pusta. Wszystkie endpointy zwracały 404.
...
```bash
az resource show \
  --ids "/subscriptions/.../providers/Microsoft.Web/sites/dvlp-ksef-func" \
...
Oczekiwany wynik: lista ~78 nazw funkcji w formacie `dvlp-ksef-func/<nazwa-funkcji>`.
...
az functionapp function list --name dvlp-ksef-func --resource-group rg-ksef --query "[].name"
```
...
```bash
# Logi na żywo
az webapp log tail --name dvlp-ksef-func --resource-group rg-ksef
...
```bash
az functionapp restart --name dvlp-ksef-func --resource-group rg-ksef
```
...
```bash
az resource show \
  --ids "/subscriptions/your-azure-subscription-id/resourceGroups/rg-ksef/providers/Microsoft.Web/sites/dvlp-ksef-func" \

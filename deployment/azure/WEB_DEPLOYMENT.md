...
- **Azure Functions API:** `your-functionapp-name` — backend (proxy przez `rewrites` w `next.config.mjs`)
...
az webapp log tail --name your-webapp-name --resource-group rg-ksef
```
...
| `API_URL` | `https://your-functionapp-name-...azurewebsites.net` | URL do Azure Functions API |
...
```bash
cd web
node scripts/build-deploy.mjs && cd .deploy && tar -acf ../deploy-package.zip * && cd .. && az webapp deploy --name your-webapp-name --resource-group rg-ksef --src-path deploy-package.zip --type zip --clean true
```

### Restart App Service

```bash
az webapp restart --name your-webapp-name --resource-group rg-ksef
```
